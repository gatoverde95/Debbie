...
# Check if Gtk initializes correctly
...
class DebInstaller(Gtk.Window):
    def __init__(self, deb_file=None):
        super().__init__(title="Debbie of Deb's")
        ...
        self.file_chooser = Gtk.FileChooserButton(title="Select a .deb package")
        ...
        # Create a horizontal box for the update and show/hide info buttons
        button_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        
        self.update_button = Gtk.Button(label="Update Information")
        self.update_button.set_sensitive(False)
        self.update_button.connect("clicked", self.on_update_clicked)
        button_box.pack_start(self.update_button, False, False, 0)

        self.hide_info_button = Gtk.Button(label="Hide Additional Info")
        ...
        # Load the .deb file if provided on startup
    ...
    def create_menu(self):
        ...
        load_item = Gtk.MenuItem(label="Load Debian Package")
        load_item.connect("activate", self.on_load_package)
        file_menu.append(load_item)

        save_item = Gtk.MenuItem(label="Save Package Information")
    ...
    def on_load_package(self, widget):  # pylint: disable=unused-argument
        ...
        filter_deb.set_name("Debian files")
    ...
    def on_save_package_info(self, widget):  # pylint: disable=unused-argument
        dialog = Gtk.FileChooserDialog(
            title="Save Package Information",
            parent=self,
    ...
    def show_about_dialog(self, widget):  # pylint: disable=unused-argument
        ...
        about_dialog.set_program_name("Debbie of Deb's")
        about_dialog.set_version("1.0 v291224a Elena")
        about_dialog.set_comments("Package installer for CuerdOS GNU/Linux.")
    ...
    def create_filter(self):
        ...
        file_filter.set_name("Debian files")
    ...
    def on_hide_info_clicked(self, widget):  # pylint: disable=unused-argument
        if self.info_hidden:
            ...
            self.hide_info_button.set_label("Hide Additional Info")
        else:
            self.hide_package_info()
            self.hide_info_button.set_label("Show Additional Info")
    ...
    def update_package_info(self, file_path, force_update=False):
        ...
        if cached_info and not force_update:
            ...
            if package_info:
                ...
                self.show_message_dialog("Error", "The .deb package has no information or could not be retrieved.")
    ...
    def hide_package_info(self):
        ...
        if file_path:
            ...
            if package_name and package_description:
                self.info_buffer.set_text(f"Package Name: {package_name}\nDescription: {package_description}\nVersion: {package_version}")
    ...
    def on_install_clicked(self, widget):  # pylint: disable=unused-argument
        ...
        if file_path:
            package_name = self.extract_package_name(file_path)
            if self.is_package_installed(package_name):
                self.show_message_dialog("Package already installed", "The package is already installed on your system.")
            else:
                self.run_command_with_output(["pkexec", "dpkg", "-i", file_path], "Installation completed.")
                self.run_command_with_output(["pkexec", "apt-get", "install", "-f", "-y"], "Dependency repair completed.")
                if not self.is_package_installed(package_name):
                    self.show_message_dialog("Installation error", "Dependencies were repaired but the package was not installed.")
                else:
                    self.show_message_dialog("Successful installation", "The package was installed successfully.")
                if package_name:
                    self.update_buttons(package_name)
    ...
    def on_uninstall_clicked(self, widget):  # pylint: disable=unused-argument
        ...
        if package_name:
            self.run_command_with_output(["pkexec", "apt", "remove", "-y", package_name], "Uninstallation completed.")
            self.update_buttons(package_name)
    ...
    def run_command_with_output(self, command, success_message):
        try:
            ...
            if process.returncode == 0:
                ...
                self.append_to_output("Error during the procedure.")
